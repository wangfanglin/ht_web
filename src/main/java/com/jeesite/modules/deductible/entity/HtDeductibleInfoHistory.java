 ...
 * @version 2020-03-24
 */
@Table(name="ht_deductible_info_history", alias="a", columns={
		...
		@Column(name="maintain_typr", attrName="maintainTypr", label="维修类型 同 工单类型"),
...
public class HtDeductibleInfoHistory extends DataEntity<HtDeductibleInfoHistory> {
	...
	private String maintainTypr;		// 维修类型 同 工单类型
	private Date offerDate;		// 确认报价时间
	private Date payDate;		// 付款时间
	private String fromType;		// 工单类型
	...
	public HtDeductibleInfoHistory() {
		this(null);
	}

	public HtDeductibleInfoHistory(String id){
		super(id);
	}
	...
	@Length(min=0, max=1, message="维修类型 同 工单类型长度不能超过 1 个字符")
	...
	@Length(min=0, max=1, message="支付方式长度不能超过 1 个字符")
